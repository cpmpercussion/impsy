...
def test_mdrnn():
  ...
  build_network(sess, compute_graph, 4, model_config["units"], model_config["mixes"], model_config["layers"])
...
def prediction_speed_test():
    ...
    def run_test(tests, config):
        ...
        with compute_graph.as_default():
            sess = tf.Session()
        net = build_network(sess, compute_graph, config["dimension"], config["units"], config["mixes"], config["layers"])
